...
/*		var o = new SerializedObject(this);
		...
		if(brokenList.Count!=0)
		{
			...
			/*if(Selection.activeObject && Selection.activeObject is GameObject && GUILayout.Button("FIX LocationObject"))
			{
				var lo = ((GameObject)Selection.activeObject).GetComponents<Runner.LocationObject>().ToList();

				while(lo.Count > 1)
				{
					DestroyImmediate(lo[lo.Count -1], true);
					lo.RemoveAt(lo.Count -1);
				}
			}*/
			if(processList != null && processList.Count > 0 && GUILayout.Button("NEXT"))
			{
			...
			if(processList != null)
			{
				GUILayout.Label(processList.Count.ToString());
			}

			/*if(GUILayout.Button("FIX ALL LOCATION OBJECTS"))
			{
				foreach(var first in processList)
				{
					var lo = first.GetComponents<Runner.LocationObject>().ToList();
